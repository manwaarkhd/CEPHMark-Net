...
def predict_on_batch(
    ...
    for index in range(len(data)):
        ...
        ldn_stats, lrn_stats = predict_step(
            data=[
                images,
                landmarks
            ],
...
def predict(
    ...
    stats = predict_on_batch(data, network,)
    results = tf.reduce_mean(tf.stack(stats), axis=0)
    print("\nvalid_loss: {:.5f} - landmark-detection-error: {:.3f} - landmark-refinment-error: {:.3f}".format(results[0], results[cfg.NUM_LANDMARKS + 1], results[2*cfg.NUM_LANDMARKS+2]), end="")
    save_statistics(results.numpy(), Paths.logs_root_path, mode="valid")
